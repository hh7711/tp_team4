...
<!-- 						@@@본문추가 구나현 -->
						...
						<table>
							<tr>
<!-- 								<th scope="col" class="displaynone">No</th>글번호 -->
                    			<th scope="col" >Item</th><!-- 상품이미지 -->
                    			<th scope="col">Subject</th><!-- 제목 마우스올리면 썸네일처럼 작게 보임 -->
                    			<th scope="col">Writer</th><!-- 작성자 -->
                    			<th scope="col">Date</th><!-- 작성날짜 -->
                    			<th scope="col">Hit</th><!-- 조회수 -->
							</tr>
							<c:forEach items="${rev_list}" var="dto">
							<tr>
<%-- 								<td>${dto.revcode}</td> --%>
								<td>
								<input type="hidden" name="revcode" value="${dto.revcode}">
									<a href="#">
							          	<img src="${dto.proimg}" alt="오류" style="width: 100px; margin: 10px;">
						        	</a>
								</td>
								<td>
									<a href="revContent?revcode=${dto.revcode}" class="aMove">
										${dto.revtitle}
									</a>
								</td>
								<td>${dto.id}</td>
